...
def get_blog_list_common_data(request, blogs_all_list):
    paginator = Paginator(blogs_all_list, settings.CACH_PAGE_BLOGS_NUMBER)  # 每7篇进行分页
    page_num = request.GET.get('page', 1)  # 获取url的页面参数（GET请求）
    page_of_blogs = paginator.get_page(page_num)
    curentr_page_num = page_of_blogs.number  # 获取当前页码
    # 获取当前页码前后各2页的页码范围
...
def blog_detail(request, blog_pk):
    ...
    response = render(request, 'blog/blog_detail.html', context)     # 响应（计数器）
